...
cipher_init(?AES_GCM, IV, Key) ->
    Ctx = crypto:aead_init(aes_gcm, Key),
    <<Nonce:64>> = ssl:random_bytes(8),
    #cipher_state{iv = IV, key = Key, state = Ctx, nonce = Nonce};
...
cipher_aead(?AES_GCM, CipherState, AAD, Fragment, Version) ->
    aead_cipher(CipherState, AAD, Fragment, Version).

aead_cipher(#cipher_state{state = Ctx, iv = IV0, nonce = Nonce} = CipherState, AAD, Fragment, _Version) ->
    <<Salt:4/bytes, _/binary>> = IV0,
    IV = <<Salt/binary, Nonce:64/integer>>,
    {Content, CipherTag} = crypto:aead_encrypt(Ctx, IV, AAD, Fragment),
...
decipher_aead(?AES_GCM, CipherState, AAD, Fragment, Version) ->
    aead_decipher(CipherState, AAD, Fragment, Version).
...
aead_decipher(#cipher_state{state = Ctx, iv=IV0} = CipherState,
	...
	case crypto:aead_decrypt(Ctx, IV, AAD, CipherText, Tag) of
...
%%--------------------------------------------------------------------
-spec anonymous_suites(ssl_record:ssl_version()) -> [cipher_suite()].
%%
