...
handle_packet(Address, Port, Packet) ->
	...
	%% expect client hello
	{[#ssl_tls{type = ?HANDSHAKE, version = {Major, _}} = Record], <<>>}
	  when Major >= 128->
...
handle_dtls_client_hello(Address, Port,
    ...
    %% io:format("CookieData: ~p~n", [CookieData]),
    %% io:format("Their Cookie: ~p~n", [Hello#client_hello.cookie]),
    %% io:format("My Cookie   : ~p~n", [Cookie]),
