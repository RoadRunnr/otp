...
controlling_process(Socket, Pid) when is_port(Socket) ->
    ?DEBUG("Socket: controlling_process: ~p ~p~n", [Socket, Pid]),
    gen_udp:controlling_process(Socket, Pid);
controlling_process(Socket, Pid) ->
    ?DEBUG("Process: controlling_process: ~p ~p~n", [Socket, Pid]),
    call(Socket, controlling_process, {self(), Pid}).

close(Socket) when is_port(Socket) ->
    ?DEBUG("Socket: close: ~p~n", [Socket]),
    gen_udp:close(Socket);
close(Socket) ->
    ?DEBUG("Process: close: ~p~n", [Socket]),
...
shutdown(Socket, _How) when is_port(Socket) ->
    ?DEBUG("SSL_UDP shutdown Socket: ~p~n", [Socket]),
    ok;
shutdown(Socket, How) ->
    ?DEBUG("SSL_UDP shutdown Process: ~p~n", [Socket]),
...
handle_ssl_info(Socket, {udp, Socket, _Address, _Port, Packet}) ->
    ?DEBUG("SSL_UDP handle_ssl_info: ~p~n", [Packet]),
...
setopts(Socket, Options) when is_port(Socket) ->
    ?DEBUG("SSL_UDP Socket setopt: ~p, ~p~n", [Socket, Options]),
    inet:setopts(Socket, Options);
setopts(Socket, Options) ->
    ?DEBUG("SSL_UDP Process setopt: ~p, ~p~n", [Socket, Options]),
...
sockname(Socket) when is_port(Socket) ->
    R = inet:sockname(Socket),
    ?DEBUG("Socket SockName: ~p", [{self(), R}]),
    R;
sockname(Socket) ->
...
terminate(Reason, State) ->
    ?DEBUG("SSL_UDP terminate: ~p, ~p, ~p~n", [self(), Reason, State]),
    ok.
...
handle_call({sockname, _, _}, _From, State = #state{socket = Socket}) ->
    ...
    ?DEBUG("Process SockName: ~p", [{self(), Reply}]),
    {reply, Reply, State};
...
handle_call({getopts, undefined, Options}, _From, State = #state{socket = Socket, mode = Mode}) ->
	...
	{ok, SocketOptions} ->
	    ?DEBUG("SSL_UDP getopts listen: ~p~n", [SocketOptions]),
...
handle_call({setopts, undefined, Options}, _From, State = #state{socket = Socket, mode = Mode}) ->
    ?DEBUG("SSL_UDP setopts listen: ~p~n", [Options]),
    Opts0 = lists:keystore(active, 1, Options, {active, true}),
    Opts = lists:keydelete(mode, 1, Opts0),
    Reply = inet:setopts(Socket, Opts),
    ?DEBUG("SSL_UDP Process handle setopt listen: ~p, ~p -> ~p~n", [Socket, Opts, Reply]),
...
handle_call(Request, _From, State) ->
    ?DEBUG("unexpected requests ~p~n", [Request]),
...
handle_info({udp, Socket, IP, InPortNo, Packet},
    ...
    ?DEBUG("SSL_UDP handle_info: ~p, ~p~n", [Socket, Packet]),
    IpKey = {IP, InPortNo},
...
handle_accept(_IpKey, _Packet, State) ->
    ?DEBUG("A accept queue is needed here......~n"),
    State.
...
socket_recv(IpKey, {_Length = 0, _Timeout = 0}, _From,
    ...
    ?DEBUG("SSL_UDP recv #2: ~p~n", [IpKey]),
    SslSocket = gb_trees:get(IpKey, IpConns0),
    ?DEBUG("SSL_UDP shutdown #3: ~p~n", [SslSocket]),
...
socket_setopts(IpKey, {SslSocketId, Options}, _From,
	...
	Active when Active /= false ->
	    ...
	    ?DEBUG("SSL_UDP dequeue: ~p~n", [queue:to_list(Queue)]),
...
socket_controlling_process(IpKey, {Old, Pid}, _From,
			   State = #state{ip_conns = IpConns0}) ->
    SslSocket = gb_trees:get(IpKey, IpConns0),
    #ssl_socket{owner = Owner} = SslSocket,
    ?DEBUG("Process: controlling_process: ~p, ~p~n", [Old, Owner]),
