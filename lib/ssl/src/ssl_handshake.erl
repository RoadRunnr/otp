...
hello(#client_hello{client_version = ClientVersion} = Hello,
	...
	true ->
		...
		_ ->
		    try handle_hello_extensions(Hello, Version, SslOpts, Session1, ConnectionStates1, Renegotiation) of
...
enc_cke(#client_psk_identity{identity = Id}, _) ->
    Len = byte_size(Id),
    <<?UINT16(Len), Id/binary>>;
enc_cke(Identity = #client_ecdhe_psk_identity{identity = undefined}, Version) ->
    enc_cke(Identity#client_ecdhe_psk_identity{identity = <<"psk_identity">>}, Version);
enc_cke(#client_ecdhe_psk_identity{identity = Id, dh_public = DHPublic}, _) ->
    Len = byte_size(Id),
...
enc_server_key(Params = #server_ecdhe_psk_params{hint = undefined}) ->
    enc_server_key(Params#server_ecdhe_psk_params{hint = <<>>});
enc_server_key(#server_ecdhe_psk_params{
		  hint = PskIdentityHint,
		  dh_params = #server_ecdh_params{
