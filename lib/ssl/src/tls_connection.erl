...
key_exchange(#state{role = client,
		    ...
		    socket = Socket, transport_cb = Transport,
		    tls_handshake_history = Handshake0} = State) ->
    Msg =  ssl_handshake:key_exchange(client, Version, {ecdhe_psk, SslOpts#ssl_options.psk_identity, ECDHKeys}),
    {BinMsg, ConnectionStates, Handshake} =
        encode_handshake(Msg, Version, ConnectionStates0, Handshake0),
    Transport:send(Socket, BinMsg),
    State#state{connection_states = ConnectionStates,
...
server_master_secret(#server_ecdhe_psk_params{
			...
			dh_params = #server_ecdh_params{curve = ECCurve, public = ECServerPubKey}},
		     State) ->
    ECDHKeys = public_key:generate_key(ECCurve),
    ecdhe_psk_master_secret(IdentityHint, ECDHKeys, #'ECPoint'{point = ECServerPubKey},
